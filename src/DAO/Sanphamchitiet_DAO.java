...
public class Sanphamchitiet_DAO implements ISanPhamChiTiet {
    ...
    public ArrayList<SanPhamChiTiet> getAllByDoanhMuc(String maKM, String maSP) {
        ...
        try {
            ResultSet rs = JDBC_Helper.query(query, maKM, maSP);
            while (rs.next()) {
                SanPhamChiTiet sp = new SanPhamChiTiet(rs.getString("MASANPHAM_CHITIET"), rs.getString("MASANPHAM"), rs.getString("MASIZE"), rs.getString("MAMAU"), rs.getString("MACHATLIEU"), rs.getString("MAHANG"), rs.getString("MALOAI"), rs.getString("MANOISANXUAT"), rs.getString("MAHINHANH"), rs.getString("TENSANPHAM_CHITIET"), rs.getString("MOTA"), rs.getDouble("GIANHAP"), rs.getDouble("GIATIEN"), rs.getInt("SOLUONG"), rs.getBoolean("TRANGTHAI"));
                list.add(sp);
            }
    ...
    public ArrayList<SanPhamChiTiet> getAllInBlackList(String maKM, String maSP) {
        ...
        try {
            ResultSet rs = JDBC_Helper.query(query, maKM);
            while (rs.next()) {
                SanPhamChiTiet sp = new SanPhamChiTiet(rs.getString("MASANPHAM_CHITIET"), rs.getString("MASANPHAM"), rs.getString("MASIZE"), rs.getString("MAMAU"), rs.getString("MACHATLIEU"), rs.getString("MAHANG"), rs.getString("MALOAI"), rs.getString("MANOISANXUAT"), rs.getString("MAHINHANH"), rs.getString("TENSANPHAM_CHITIET"), rs.getString("MOTA"), rs.getDouble("GIANHAP"), rs.getDouble("GIATIEN"), rs.getInt("SOLUONG"), rs.getBoolean("TRANGTHAI"));
                list.add(sp);
            }
    ...
    public int getSoLuongSanPham(String maSP, String maHD) {
        ...
        try {
            ResultSet rs = JDBC_Helper.query(query, maSP, maHD);
            if (rs.next()) {
                return rs.getInt(1);
            }
            rs.getStatement().getConnection().close();
